...
envs
  ...
  .action(async (options: { output: OutputFormat }) => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/api/v1/projects/default/envs`, {
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `List environments failed: HTTP ${response.status} ${response.statusText}`
        );
      }

      const envs = await parseJsonResponse(response, {
...
envs
  ...
  .action(async (name: string, options: { output: OutputFormat }) => {
    ...
    try {
      ...
      const [collectionsRes, managedUsersRes, syncsRes, runsRes] =
        await Promise.all([
          fetch(`${baseUrl}/api/v1/projects/default/envs/${name}/collections`, {
            headers: { Authorization: `Bearer ${apiKey}` },
          }),
          fetch(
            `${baseUrl}/api/v1/projects/default/envs/${name}/managed-users?countOnly=true`,
            {
              headers: { Authorization: `Bearer ${apiKey}` },
            }
          ),
          fetch(
            `${baseUrl}/api/v1/projects/default/envs/${name}/syncs?syncStatus=RUNNING`,
            {
              headers: { Authorization: `Bearer ${apiKey}` },
            }
          ),
          fetch(
            `${baseUrl}/api/v1/projects/default/envs/${name}/runs?runStatus=RUNNING`,
            {
              headers: { Authorization: `Bearer ${apiKey}` },
            }
          ),
      ...
      const managedUsersData = managedUsersRes.ok
        ? await managedUsersRes.json()
        : { count: 0 };
